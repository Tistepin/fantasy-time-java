...
public class WorksServiceImpl extends ServiceImpl<WorksDao, WorksEntity> implements WorksService {
    ...
    @Override
    @Transactional
    public void uploadWork(WorksTo worksTo, HttpServletRequest request) {
        // 转换数据把to数据转换到Entity中
        ...
        worksUploadEntity.setWorksId(worksEntity.getId());
    ...
    public List<WorksVo> getWorksPage(Integer worksType, List<Long> worksList, Integer page, Integer limit) {
        ...
        IPage<WorksEntity> pages = this.page(
                ...
                new QueryWrapper<WorksEntity>().eq("works_type", worksType).in("id",worksList)
        );
        // 转移数据
    ...
    public List<WorksVo> getRecommendedToday(Integer worksType) {
        ...
        * */
        ...
        this.baseMapper.selectList(new QueryWrapper<WorksEntity>().select("works_category").eq("delete_status", 1).in("id", worksIds)).forEach(item -> {
            // 获取该作品的分类  用,分割set收集
